...
http.createServer(function (req, res) {
    ...
    } else if (postfix == ".jpeg" || postfix == ".jpg" || postfix == ".png") {
        showPic(res, urlPathName);
    } else if (postfix == ".js") {
        showJs(res, fileName);
    } else if (urlPathName == "/list/getdata") {
        getData(res);
    } else if (urlPathName == "/detail") {
        showChapter(res);
    } else if (urlPathName == "/getDetail") {
        changeChapter(req, res);
    } else if (urlPathName == "/login/") {
        showLogin(res);
    } else if (urlPathName == "/login/check") {
        checkId(req, res);
    } else if (urlPathName == "/listmanager/") {
        showlistmag(res);
    } else if (urlPathName == "/addChapter/") {
        showaddCp(res);
...
function showCss(res, fileName) {
    let cssPath = path.join(__dirname, "..//css/" + fileName);
    let fileContent = fs.readFileSync(cssPath);
    res.writeHead(200, { "Content-Type": "text/css" });
    res.end(fileContent);
}

function showPic(res, urlPathName) {
    if (urlPathName.substring(0, 6) == "/login") {
        let fileName = urlPathName.replace("/login", "");
        let imgPath = path.join(__dirname, "../" + fileName);
        let imgContent = fs.readFileSync(imgPath);
        res.writeHead(200, { "Content-Type": "image/png" });
        res.end(imgContent);
    } else if (urlPathName.substring(0, 6) == "/listm") {
        let fileName = urlPathName.replace("/listmanager", "");
        let imgPath = path.join(__dirname, "..//images" + fileName);
        let imgContent = fs.readFileSync(imgPath);
        res.writeHead(200, { "Content-Type": "image/png" });
        res.end(imgContent);
    } else {
        let fileName = urlPathName.replace("/list", "");
        let imgPath = path.join(__dirname, "../" + fileName);
        let imgContent = fs.readFileSync(imgPath);
        res.writeHead(200, { "Content-Type": "image/png" });
        res.end(imgContent);
...
function showJs(res, fileName) {
    let jsPath = path.join(__dirname, fileName);
    let fileContent = fs.readFileSync(jsPath);
    res.writeHead(200, { "Content-Type": "text/javascript" });
    res.end(fileContent);
}

function getData(res) {
    let str = JSON.stringify(chapterList);
...
function changeChapter(req, res) {
    let urlPath = req.url;
    let chId = urlPath.charAt(urlPath.length - 1);
    for (let i = 0; i < chapterList.length; i++) {
        if (chId == chapterList[i].chapterId) {
            let str = JSON.stringify(chapterList[i]);
            res.writeHead(200, { "Content-Type": "text/plain" });
            res.end(str);
        }
    }
...
function showaddCp(res) {
    let filePath = path.join(__dirname, "..//addChapter.html");
    let fileContent = fs.readFileSync(filePath);
    res.writeHead(200, { "Content-Type": "text/html" });
    res.end(fileContent);
}

function addCp(req, res) {
    let str = "";
    req.on("data", function (data) {
        str += data
    })
    req.on("end", function () {
        let content = querystring.parse(str);
        let day = new Date();
        var newCp = {
